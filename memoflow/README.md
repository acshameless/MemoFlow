...
- 🗂️ **命名空间管理**：支持多仓库管理，每个仓库作为独立命名空间（类似 k8s namespace）
- 🧭 **资源导航**：TUI 中支持按 Area/Category 过滤，提供 k9s 风格的资源视图
...
# 在 TUI 中：
# - 按 A/a 选择 Area 进行过滤
# - 按 G/g 选择 Category 进行过滤
# - 按 S 查看 schema 配置
# - 按 n 创建新文件，m 移动文件，e 打开编辑器等
...
- `mf init [path]` - 初始化 MemoFlow 仓库（自动注册到全局注册表）
- `mf version` - 显示版本信息

### 仓库管理（Namespace Management）
...
- `mf` - 启动交互式 TUI 仪表板（推荐用于日常操作）
  - **TUI 资源导航**（k9s 风格）：
    - `A` / `a` - 选择 Area，按 Area ID 过滤文件列表
    - `G` / `g` - 选择 Category（需先选择 Area），按 Category 范围过滤
    - `S` - 查看完整的 schema 配置
    - Context Bar 显示当前 Namespace / Area / Category / View
  - 支持实时操作：修改类型、状态、移动文件、打开编辑器等
- `mf status` - 显示仓库统计数据和文件列表（静态输出）
  - 支持 `--limit`, `--all`, `--type`, `--status` 等过滤选项
...
- 在交互式 TUI（运行 `mf`）中按 `S` 键可查看完整的 schema 配置
...
MemoFlow 维护一个全局仓库注册表（`~/.memoflow/repos.json`），用于管理多个仓库：
  ...
  "repos": [
    {
      ...
      "path": "/Users/username/my-second-brain"
    },
    {
      "name": "work-notes",
...
- 每个仓库作为独立的命名空间（类似 k8s namespace）
- `mf init` 时自动注册到全局注册表
- 使用 `mf repo list` 查看所有注册的仓库
- 使用 `mf repo info [name]` 查看仓库详细信息
...
pytest tests/test_repo_registry.py  # 命名空间管理测试
pytest tests/test_cli.py            # CLI 端到端测试
